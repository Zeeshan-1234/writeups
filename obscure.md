...
## SSH
If we try to login with ssh, we get `Permission denied (publickey).` which means password authentication has been disabled for 
ssh.
...
using `admin@antisoft.thm` which is simpler but needs guess work, or going to "manage database" and creating a new one with our
master password.
...
# Docker
After getting a reverse shell, we see that we're in a docker container. There's a custom binary `/ret`, we download this and
...
So, we need to find the offset and then place the value of `win` function in "rsp", I like using gdb(with gef/peda) for this.
In gdb, run `pattern create 200`, copy the output, `run` to run the program and paste. We see the program crash and if we do 
...
Running `nmap 172.17.0.0/24` we the host `172.17.0.1` has port 4444 open, which isn't accessible from the outside. Using nc to connect to the port we see that the `ret` binary is running over this port. We could forward this port to our machine and directly use pwntools for the exploitation which makes this process a lot neater and easier.
...
Running our usual checks, we see that there's no win function in this binary, no stack canary and no pie but we can cause a overflow. This setup should point to a ret2libc attack which is a relatively complex exploit, so I won't be going into too much detail explaining everything but I highly recommend doing the research on your own.
